// super这个关键字，既可以当作【函数】使用，也可以当作【对象】使用。在这两种情况下，它的用法完全不同。
...
class B extends A{
  ...
  get m() {
    return super.p;     // super:A.proptotype
  }
  get n() {
    return this.p;
  }
...
// 即：A.prototype.method中的this均指向子类实例
class A{
  constructor () {
    this.x =1;
  }
...
class B extends A{
  ...
  m(){
    super.print();  // equals to: A.prototype.print.call(this);
  }
}
let b = new B();
b.m();      //2
...
class Child extends Parent{
  static myMethod(msg){
    super.myMethod(msg);      // super为父类A，而非A.prototype
  }

  myMethod(msg){
    super.myMethod(msg);      // super为A.prototype
  }
}
Child.myMethod(1);
let child = new Child();  // static 1
...
class B extends A {
  constructor() {
    super();
    console.log(super.valueOf() instanceof B);   // error
  }
}
let b = new B();
