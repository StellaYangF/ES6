...
/* 
  ...
  （4） Reflect对象的方法与Proxy对象的方法一一对应，只要是Proxy对象的方法，
        ...
        完成默认行为，作为修改行为的基础。也就是说，不管Proxy怎么修改默认行为，
        你总可以在Reflect上获取默认行为。
*/ 
...
// 4.1 example
Proxy(target, {
  set(target, name, value, receviver){
    let success = Reflect.set(target, name, value, receviver);
    if(success){
      console.log(`propert ${name} on ${target} set to ${value}`);
    }
    return success;
  }
})
...
// new method
Reflect.apply(Math.floor, undefined, [1.74]);//1
